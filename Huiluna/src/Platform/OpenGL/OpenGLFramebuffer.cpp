...
namespace Huiluna {
	...
	namespace Utils {
		...
		static void AttachColorTexture(uint32_t id, int samples, GLenum format, uint32_t width, uint32_t height, size_t index)
		{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			HL_CORE_ASSERT(m_ColorAttachments.size() <= 4, "");
		...
		HL_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebnuffer is incomplete!");
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			HL_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
